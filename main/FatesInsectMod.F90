module FatesInsectMod
  ...
  use FatesInterfaceTypesMod         , only : hlm_current_year, hlm_current_month, hlm_current_day, hlm_freq_day
...
!==================================================================================================
	subroutine WPBSim(max_airTC, min_airTC, Parents, FA, OE, OL1, OL2, &
		...
		real(r8), intent(inout) :: OPare(2**8)	
		real(r8), intent(inout) :: NewParentstm1
		...
		real(r8), intent(inout) :: ActiveParents
		real(r8), intent(inout) ::Pare
		...
		real(r8), intent(inout) :: NtGEQ317
		real(r8), intent(inout) ::NtGEQ00              ! initial susceptible host trees in the 20+ cm dbh size class
		...
		!!! Development rates. 
		!!!! See Robbins et al., 2021 for more details on parameterization of these development rates
		...
		real(kind = 8), parameter :: sigma2=0.272     
		real(kind = 8), parameter :: Rmax2=0.036            
		real(kind = 8), parameter :: VTmx2=33.982
		real(kind = 8), parameter :: VTmn2= 5.241
		real(kind = 8), parameter :: k2= 796.601
		real(kind = 8), parameter :: dm2 = 6.604
		...
		real(kind = 8), parameter :: sigma5=.2     
		...
		!!! These are bins for the daily relationship of temperature and development
		...
		real(kind = 8) :: muA               ! mean of the log transformed random development rate in teneral adult stage
...
!---------------Empirical transformation of air temperature to under bark temperature Ponderosa pine---------!
	...
	! The mu parameter of the lognormal distribution is given by the natural logarithm of the median
		...
		if(med4 > 0.0_r8) then	
			mu4 = log(med4*deltat)   ! for pupae
		end if 	

		if(med5 > 0.0_r8) then	
			mu5 = log(med5*deltat)  ! for teneral adults
		end if 	
		if(medA > 0.0) then        
            muA = log(medA*deltat)  ! for teneral adults
        end if  
	...
	!	 If temperatures are cold, all of the flying beetles die. This prevents them
		...
		if(fouram <= -12.2)then        !pg 90 Miller and Keen
			Parents=Parents*.1
			OPare=OPare*.1
		...
		if(fouram <= -20.6)then !pg 90 Miller and Keen
			OE=OE*.5
			E=E*.5
		end if
...
!==================================================================================================================	
		subroutine WPBAttack(NtGEQ317, NtGEQ00,Bt, FA, Parents, r1,x0,x1,CWD,x2,SizeFactor,In_PopN,EndPopn)	
		 ...
		 ! the number of beetles in flight and the amount of drought experienced by the trees and their size. 
			...
			real(r8), intent(inout) :: NtGEQ00              ! initial susceptible host trees in the small dbh size class
			real(r8), intent(inout) :: NtGEQ317             ! initial susceptible host trees in the large dbh size class
			...
			real(r8), intent(in) :: r1                      ! controls the tree loss rate
			real(r8), intent(in) :: x0                      ! controls proportion of beetles become parents
			real(r8), intent(in) :: x1                      ! controls the tree loss rate
			real(r8), intent(in) :: CWD                      ! controls proportion of beetles become parents
			...
			real(r8) :: Bt2                     	    ! an updated value for the beetles
			real(r8) :: Bt3  
			real(r8) :: Ntp1GEQ317                 	    ! updated susceptible host trees in the 20+ cm dbh size class
			real(r8) :: Ptp1GEQ20  
			real(r8) :: Ntp1GEQ00			
			! updated parent beetles the 20+ cm dbh size class
			...
			if(NtGEQ317 >0.0_r8) then
				...
				ibeta=1_r8-((q1)/(1+phi1))
				! The expected number of beetles to attack each the larger tree type is determined
				Ps=(1_r8-ibeta)**SizeFactor
				! No less than 50% of beetles  will attack the perferred host (No-preference)
				if(Ps <.5_r8) then
					Ps=.5_r8
				end if 
				Bt2= Bt*Ps
				...
				ibeta2=1_r8-(q2/(1_r8+phi1)) 
				! The remaining population 
				Ntp1GEQ317 = (ibeta2 * NtGEQ317)
				!Little trees
				! Number of beetles to attack smaller trees
				Bt3 = Bt *(1_r8-Ps)    
				q3 = (r1*Bt3)/(NtGEQ00)     
				ibeta3=1_r8-(q3/(1_r8+phi2))
				! The surviving Small Trees
				Ntp1GEQ00 = (ibeta3 * NtGEQ00)
				Ptp1GEQ20 = phi1*(NtGEQ317-Ntp1GEQ317)+phi2*(NtGEQ00-Ntp1GEQ00)
				!(Beetles die in attempted attacks)
				Bt=0
				!! Handleing na
				! Ensure more trees aren't killed than exist. 
				if(Ntp1GEQ00<0_r8)then
					Ntp1GEQ00=0_r8
				end if 
				if(Ntp1GEQ317<0_r8)then
					Ntp1GEQ317=0_r8
				end if
				if(isnan(Ntp1GEQ317))then
					Ntp1GEQ317=0_r8
				end if 
				...
				if (isnan(phi1*(NtGEQ317-Ntp1GEQ317))) then
					Ptp1GEQ20=phi2*(NtGEQ00-Ntp1GEQ00)
				end if 
				if (isnan(phi2*(NtGEQ00-Ntp1GEQ00))) then
					Ptp1GEQ20=phi1*(NtGEQ317-Ntp1GEQ317)
				end if 
